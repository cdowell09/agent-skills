...
- `receipt.dogfood` was first described by #11 as an `AGENT_SKILLS_RECEIPT` one-line JSON form; the dogfooding spec now uses the common marker-block envelope above. Resolved.
...
| Receipt emission form | dogfooding (`AGENT_SKILLS_RECEIPT` one-line JSON) vs work-execution, human-gate, project-research (marker block) | The marker block is canonical for every family; the dogfooding spec was aligned the same day. | One parser for #15 and for any future reconciler. |
