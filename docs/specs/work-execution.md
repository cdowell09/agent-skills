...
- **Standalone worker.** Unattended: claim immediately after selection. Interactive: select, print the summary, confirm, **then** claim, **then** create the worktree. Claiming first would post and release a comment whenever the human declines, whereas the claim step after confirmation re-reads the active claim, so a collision during the pause yields `claim-held` and a fresh selection, never duplicate work. The human-gate family uses the same timing; the capability-configuration reconciliation log records this as the shared rule.
